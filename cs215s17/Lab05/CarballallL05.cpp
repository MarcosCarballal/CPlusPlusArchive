...
{ // This function writes a short report about the words
  ...
  // output stream passed to this function.
	...
	// Creates variables to store the totals
	int totalvowels = 0, totalconsonants = 0,
		totaldigits = 0, totalspecialchars = 0;
	// Parses through words and adds to total
	for (int i = 0; i < count; i++)
	{
	...
	// Prints the totals
        ...
        output << setw (8) << right << totalconsonants;
        output << setw (8) << right << totaldigits;
        output << setw (8) << right << totalspecialchars;
        output << endl;
